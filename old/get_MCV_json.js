...
const MCV = {};
 ...
 for(let target_id in recipes_fest){
    target_id = recipes_fest[target_id]
...
MCV[target_id] = {
    pattern : xpattern,
    result : get_icon_i(target_rec["result"].length ? target_rec["result"][0]["item"]:target_rec["result"]["item"],target_rec["result"].length ? target_rec["result"][0]["data"]:target_rec["result"]["data"] || 0)
};

}

fs.writeFileSync("./MCV.json",JSON.stringify(MCV))